...
export class TasksController {
  public createTask = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: `Erro ao salvar a tarefa no MongoDB: ${error}` })
    }
  }
  ...
  public deleteTask = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: `Erro ao deletar a tarefa no MongoDB: ${error}` })
    }
  }
  ...
  public updateTask = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: `Erro ao atualizar a tarefa no MongoDB: ${error}` })
    }
  }
}
